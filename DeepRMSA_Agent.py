"""
1. Python code for DeepRMSA Agent for Elastic Optical Network.
2. We used the TensorFlow 2.x for implementation.
"""
...
class DeepRMSA_Agent():
	...
	def __init__(self,
        ...
        Args:
            ...
            trainer: The trainer that the agent will use to train its local network.
            linkmap: A map of the network links.
            LINK_NUM: The number of links in the network.
            NODE_NUM: The number of nodes in the network.
            SLOT_TOTAL: The total number of slots in each link.
            k_path: The number of candidate paths for each source-destination pair.
            M: The number of requests that the agent can handle at a time.
            lambda_req: The arrival rate of requests.
            lambda_time: The service time distribution of requests.
            len_lambda_time: The length of the service time distribution.
            gamma: The discount factor.
            episode_size: The number of steps in an episode.
            batch_size: The batch size for training the local network.
            Src_Dest_Pair: A list of source-destination pairs.
            Candidate_Paths: A dictionary of candidate paths for each source-destination pair.
            num_src_dest_pair: The number of source-destination pairs.
            model_path: The path to the saved model file.
            global_episodes: A global episode counter.
            regu_scalar: The regularization scalar.
            x_dim_p: The dimension of the policy network.
            x_dim_v: The dimension of the value network.
            n_actions: The number of possible actions.
            num_layers: The number of layers in the local network.
            layer_size: The size of each layer in the local network.
            model2_flag: A flag indicating whether to use the model 2 architecture.
            nonuniform: A flag indicating whether to use the nonuniform arrival rate of requests.
            prob_arr: An array of probabilities for the nonuniform arrival rate of requests.
        """
		...
		self.global_episodes.assign_add(1, name=self.name)

		self.episode_rewards = []
		self.episode_blocking = []
		self.episode_mean_values = []
		self.summary_writer = tf.summary.create_file_writer("train_" + self.name)
        ...
        # Create an operation to update the target network.
		self.update_local_ops = self.update_target_graph('global', self.name)

        # Initialize the slot map and request set.
		self.slot_map = [[1 for x in range(self.SLOT_TOTAL)] for y in range(self.LINK_NUM)]
		self.slot_map_t = [[0 for x in range(self.SLOT_TOTAL)] for y in range(self.LINK_NUM)]
	...
	def update_target_graph(self, from_scope, to_scope):
		"""Gets the kth path from src to dst from the Candidate_Paths dictionary.

		Args:
			src: The source node.
			dst: The destination node.
			Candidate_Paths: A dictionary of candidate paths from each source node to each destination node.
			k: The index of the path to return.
		Returns:
	...
	def _get_path (self, src,dst,Candidate_Paths,k): # get path k of from src->dst
		"""Updates the target network with the weights of the local network.

		Args:
			from_scope: The scope of the local network.
			to_scope: The scope of the target network.
		Returns:
			A list of operations that assign the values of the local network variables to the target network variables.
		"""
		if src == dst:
			tf.print('error: _get_path()')
			path = []
		else:
	...
	def calclink(self, p):  # map path to links
		"""Maps a path to a list of links.

		Args:
			p: A list of nodes in the path.
		Returns:
			A list of links in the path."""
	...
	def get_new_slot_temp(self, slot_temp, path_link, slot_map):
		"""Updates the slot map for a given path.

		Args:
			slot_temp: A list of available slots in the link.
			path_link: A list of links in the path.
			slot_map: The current slot map.
		Returns:
			A list of updated available slots for the link.
		"""
		for i in path_link:
	...
	def mark_vector(self, vector,default):
		"""Marks a vector to find the longest continuous sequence of default values.

		Args:
			vector: A list of values.
			default: The default value.
		Returns:
			A tuple of (flag, slotflag, slotscontinue), where:
			flag: A flag indicating whether the vector contains a continuous sequence of default values.
			slotflag: A list of indices of the first slot in each continuous sequence of default values.
			slotscontinue: A list of lengths of each continuous sequence of default values.
		"""
		le = len(vector)
		flag = 0
		slotscontinue = []
	...
	def judge_availability(self, slot_temp,current_slots,FS_id):
		"""Judges the availability of the slots in a link for a given flow size.

		Args:
			slot_temp: A list of available slots in the link.
			current_slots: The flow size.
			FS_id: The ID of the flow size.
		Returns:
			A tuple of (flag_availability, fs, fe), where:
			flag_availability: A flag indicating whether the link has enough contiguous slots to accommodate the flow size.
			fs: The start slot of the contiguous slots.
			fe: The end slot of the contiguous slots.
		"""
	...
	def update_slot_map_for_committing_wp(self, slot_map, current_wp_link, current_fs, current_fe, slot_map_t, current_TTL): # update slotmap, mark allocated FS' as occupied
		"""Updates the slot map to mark the slots allocated to a committed working path as occupied.

		Args:
			slot_map: The current slot map.
			current_wp_link: A list of links in the committed working path.
			current_fs: The start slot of the allocated slots.
			current_fe: The end slot of the allocated slots.
			slot_map_t: The target slot map.
			current_TTL: The TTL of the allocated slots.

		Returns:
			A tuple of (slot_map, slot_map_t), where:
			slot_map: The updated slot map.
			slot_map_t: The updated target slot map.
		"""
	...
	def update_slot_map_for_releasing_wp(self, slot_map, current_wp_link, current_fs, current_fe): # update slotmap, mark released FS' as free
		"""Updates the slot map to mark the slots released by a working path as free.

		Args:
			slot_map: The current slot map.
			current_wp_link: A list of links in the released working path.
			current_fs: The start slot of the released slots.
			current_fe: The end slot of the released slots.

		Returns:
			The updated slot map.
		"""
		for ll in current_wp_link:
			for s in range(current_fs,current_fe + 1):
	...
	def release(self, slot_map, request_set, slot_map_t, time_to):  # update slotmap to release FS' occupied by expired requests
		"""Updates the slot map to release the slots occupied by expired requests.

		Args:
			slot_map: The current slot map.
			request_set: A dictionary of requests, where the keys are the request IDs and the values are lists containing the following information:
			* A list of links in the working path of the request.
			* The start slot of the allocated slots.
			* The end slot of the allocated slots.
			* The TTL of the allocated slots.
			slot_map_t: The target slot map.
			time_to: The time to release the slots.

		Returns:
			A tuple of (slot_map, request_set, slot_map_t), where:
			slot_map: The updated slot map.
			request_set: The updated request set.
			slot_map_t: The updated target slot map.
		"""
		if request_set:
            # update slot_map_t
	...
	def cal_len(self, path):
		"""Calculates the length of a path in terms of the number of slots required.

		Args:
			path: A list of nodes in the path.

		Returns:
			The length of the path in terms of the number of slots required.
		"""
		path_len = 0
	...
	def cal_FS(self, bandwidth, path_len):
		"""Calculates the number of flow sizes required for a given bandwidth and path length.

		Args:
			bandwidth: The bandwidth in Mbps.
			path_len: The length of the path in terms of the number of slots required.

		Returns:
			The number of flow sizes required.
		"""
		if path_len <= 625:
	...
	def discount(self, x):
		"""Discounts a sequence of rewards.

		Args:
			x: A numpy array of rewards.
			gamma: The discount factor.

		Returns:
			A numpy array of discounted rewards.
		"""
	...
	def train(self, espisode_buff, sess, value_est):
		...
		Args:
			espisode_buff: A list of episode buffers.
			sess: A TensorFlow session.
			value_est: The estimated value of the current state.

		Returns:
			A tuple of (value_loss, policy_loss, entropy, grad_norms_policy, grad_norms_value, var_norms_policy, var_norms_value, regu_loss_policy, regu_loss_value).
		"""
