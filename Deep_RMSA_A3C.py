...
master_network = AC_Net(
    scope='global',
    trainer=None,  # You can set this to the trainer if needed
    x_dim_p=x_dim_p,
    x_dim_v=x_dim_v,
    n_actions=n_actions,
...
num_agents = multiprocessing.cpu_count()  # Set workers to the number of available CPU threads
max_cpu = 4  # Define the max_cpu value as needed
if num_agents > max_cpu:
    num_agents = max_cpu  # Cap the number of agents to max_cpu CPUs
agents = []
...
saver = tf.train.Checkpoint(agents=agents, optimizer=trainer)
